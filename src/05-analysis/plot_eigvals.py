...
def main(input_dir, output_dir):
    ...
    MD_EIGVALS_PATH   = "tmp/eigvals.dat"
    ENM_EIGVALS_PATH  = "tmp/eigenvals.csv"
    ENM_FREQ_PATH   = "tmp/mode.frequencies"

    eigvals_md = pd.read_fwf(MD_EIGVALS_PATH, infer_nrows=7000, header=None)
    eigvals_md.columns = ['mode_number', 'eigenvalue_0']
    # eigvals_md['eigenvalue_0'] = np.sqrt(eigvals_md['eigenvalue_0'].to_numpy())
    ...
    ax = axs[0][0]
    ax.scatter(eigvals_md[:plot_modes].index, eigvals_md[:plot_modes]['eigenvalue_0'],
        label='MD', color=color_cycler[0], s=5)
    ax.scatter(eigvals_enm[:plot_modes].index, eigvals_enm[:plot_modes]['eigenvalue_0'],
    ...
    eigvals_ratio = eigvals_md['eigenvalue_0'] / eigvals_enm['eigenvalue_0']
    ax = axs[0][1]
    ...
    ax = axs[1][0]
    ax.hist(eigvals_md['eigenvalue_0'][:950].to_numpy(), **hist_kwargs, 
    ...
    ax = axs[1][1]
    ax.hist(eigvals_enm['eigenvalue_0'][:950].to_numpy(), **hist_kwargs, 
    ...
    eigvals_ratio = eigvals_md['eigenvalue_0'] / freq_enm['eigenvalue_0']
    ax = axs[0][1]
    ax.scatter(eigvals_ratio[:plot_modes].index, eigvals_ratio[:plot_modes],
        color=color_cycler[2], s=5)
    ax.set_ylabel("$\lambda_{MD}/\lambda_{ENM}$")
    ...
    plt.savefig(join_paths(output_dir, "eigvals.freq.pdf"), bbox_inches='tight')
    plt.savefig(join_paths(output_dir, "eigvals.freq.png"), bbox_inches='tight')
