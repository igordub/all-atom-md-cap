...
# Run antechamber 
antechamber -i ${PDB_PRO_DIR}/ligand.chainA.pdb -fi pdb -o ${ANTECHAM_DIR}/ligand.chainA.mol2 -fo mol2 \
    -s 2 -c bcc -at gaff2 \
    -nc -1 \
    -m 2
rm ANTECHAMBER_*.AC ANTECHAMBER_*.AC0 ATOMTYPE.INF sqm.*

antechamber -i ${PDB_PRO_DIR}/ligand.chainB.pdb -fi pdb -o ${ANTECHAM_DIR}/ligand.chainB.mol2 -fo mol2 \
    -s 2 -c bcc -at gaff2 \
    -nc -1 \
    -m 2
rm ANTECHAMBER_*.AC ANTECHAMBER_*.AC0 ATOMTYPE.INF sqm.*
