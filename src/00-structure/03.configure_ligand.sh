...
# Ligand charge: -1
# Multiplicity: 2
antechamber -i ${PDB_PRO_DIR}/ligand.pdb -fi pdb -o ${ANTECHAM_DIR}/ligand.mol2 -fo mol2 \
    -s 2 -c bcc -at gaff2 \
    -nc -1 \
    -m 2
rm ANTECHAMBER_*.AC ANTECHAMBER_*.AC0 ATOMTYPE.INF sqm.*

parmchk2 -i ${ANTECHAM_DIR}/ligand.mol2 -f mol2 -o ${ANTECHAM_DIR}/ligand.frcmod -s gaff2
