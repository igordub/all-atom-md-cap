...
# Two identical ligands on each chain
cp ${PDB_INT_DIR}/ligand.pdb ${PDB_PRO_DIR}/ligands.pdb

# Extract ligand separately
grep -e '^HETATM.\{14\} A ' ${PDB_INT_DIR}/ligand.pdb > ${PDB_PRO_DIR}/ligand.chainA.pdb
grep -e '^HETATM.\{14\} B ' ${PDB_INT_DIR}/ligand.pdb > ${PDB_PRO_DIR}/ligand.chainB.pdb
